...
func ConfigureOpenTelemetry(conf Config) (func(), error) {
	kengineApiKey := getEnv("KENGINE_API_KEY", conf.KengineApiKey)

	if kengineApiKey == "" {
		log.Printf("KENGINE_API_KEY not set, not configuring OpenTelemetry")
		return nil, nil
	}
